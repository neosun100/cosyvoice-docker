...
def cosyvoice_example():
    ...
    for i, j in enumerate(
        ...
        torchaudio.save(
            "cross_lingual_{}.wav".format(i), j["tts_speech"], cosyvoice.sample_rate
        )
    # vc usage
    for i, j in enumerate(
        cosyvoice.inference_vc(
            "./asset/cross_lingual_prompt.wav", "./asset/zero_shot_prompt.wav"
        )
    ):
...
def cosyvoice2_example():
    ...
    for i, j in enumerate(
        ...
        torchaudio.save(
            "zero_shot_bistream_{}.wav".format(i),
            j["tts_speech"],
            cosyvoice.sample_rate,
        )
